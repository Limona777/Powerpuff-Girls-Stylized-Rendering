Shader "PPG/BasicFurShader" {
    Properties {
        ...
        _SpecColor2("Specular Color 2", Color) = (0.8,0.8,0.8,1)
        _SpecInfo("Spec Info (X:Pow1 Y:Shift1 Z:Pow2 W:Shift2)", Vector) = (4,0.1,8,0.2)
    }
...
fixed4 _SpecColor1, _SpecColor2;
float4 _SpecInfo;

struct appdata_fur { float4 vertex : POSITION; float3 normal : NORMAL; float4 tangent : TANGENT; float2 texcoord : TEXCOORD0; };
struct v2f_fur { float4 pos : SV_POSITION; float2 uv_FurTex : TEXCOORD0; float2 uv_Noise : TEXCOORD1; fixed4 lightMul : TEXCOORD2; fixed3 spec : TEXCOORD3; fixed fresnel : TEXCOORD4; };
...
float StrandSpecular(float3 T, float3 V, float3 L, float exponent){
    ...
    float dirAtten = smoothstep(-1,0,dotTH); return dirAtten * pow(sinTH,exponent);
}
...
v2f_fur vert_fur (appdata_fur v) {
    ...
    o.spec = (_SpecColor1.rgb*spec1+_SpecColor2.rgb*spec2)*FURSTEP*2*nl;
    return o;
}
fixed4 frag_fur (v2f_fur i) : SV_Target {
    ...
    fixed3 finalColor = albedo*i.lightMul.rgb*occColor + i.spec*alpha + i.fresnel*_FresnelColor.rgb;
